...
import com.example.mydailyroutine.ui.components.DateNavigator
...
@Composable
fun TimelineScreen(viewModel: TimelineViewModel, access: NotificationAccess,
    ...
    val onAction: (TimelineAction) -> Unit = remember(viewModel, haptics) { { action ->
        if (action !is TimelineAction.ToggleComplete) haptics.tap()
...
private fun periodTitle(data: TimelineContent): String = when (data.mode) {
    ...
    TimelineMode.WEEK -> TimelineViewModel.range(data.date, data.mode).let { (first, last) -> stringResource(R.string.date_range,
    ...
    TimelineMode.YEAR -> TimelineViewModel.range(data.date, data.mode).first.year.let { stringResource(R.string.academic_year, it, it + 1) }
}
