...
/**
 * The app's audio vocabulary, drawn from the brand's two sound catalogs — the fifteen-sound
 * interface pack (`sound_*`) and the ten-sound click pack (`click_*`) — and paired with the haptic
 * vocabulary in [com.example.mydailyroutine.core.designsystem.haptics.RoutineHaptics].
 *
 ...
 *
 * * **addToDay** — click pack 2, on the editor's „Dodaj v moj dan“ save with the success haptic:
 *   the day accepted a new block.
 ...
 *   haptic.
 * * **topAction** — click pack 6, on the four top-bar icon buttons (načrtovanje, naloge, cilji,
 *   nastavitve), with the press haptic.
...
class RoutineSounds internal constructor(context: Context, private val enabled: State<Boolean>) {
    ...
    private val addToDayId = pool.load(context, R.raw.click_2, 1)
    ...
    private val topActionId = pool.load(context, R.raw.click_6, 1)
    private val selectId = pool.load(context, R.raw.sound_1, 1)

    /** „Dodaj v moj dan“: the day accepted a new block. Click pack 2, success haptic. */
    ...
    /** A top-bar icon button was pressed. Click pack 6, press haptic. */
