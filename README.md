...
- Shared **Deep Oceanic Slate** palette: `#090D16` → `#26324F`, teal interactions, cyan live time, violet focus and separate success/warning/error roles. `RoutineColors` is the source of truth; `tools/derive_palette.py --write` generates its native XML mirror and `--check` detects drift. `tools/check_contrast.py` verifies actual opaque/composited combinations at WCAG AA, with AAA category content. Decorative tertiary text is not used for small labels. See [the palette audit](docs/audits/2026-09-20-palette-and-glass.md).
- Liquid-glass chrome uses the pinned Backdrop **1.0.0** API, separated `GlassStyles`, 4–10 dp blur, bounded refraction, `vibrancy()` without extra exposure/contrast, restrained directional rims and local blue/violet ambient light. Content cards remain opaque. Scrolling chrome uses a stronger measured tint for readable labels even over white content; Android 7–11 receives an opaque fallback. Existing reduced-motion, back navigation and touch feedback are preserved.
