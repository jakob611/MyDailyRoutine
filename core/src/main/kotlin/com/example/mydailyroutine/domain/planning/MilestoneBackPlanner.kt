...
data class MilestonePlan(val blocks: List<PreparationBlock>, val unplacedChunks: List<Int>, val unreservedMinutes: Int, val virtualDeadline: LocalDate)
...
class MilestoneBackPlanner {
    fun plan(initialDate: LocalDate, deadline: LocalDate, estimatedEffortHours: Double, terminal: Boolean,
        ...
        val rawTotal = ceil(estimatedEffortHours * 60).toInt()
        val total = ceil(rawTotal * velocity.coerceIn(0.5,2.5)).toInt()
        ...
        chunks.forEachIndexed { index, duration ->
            ...
            if (selected == null) missing += duration else {
