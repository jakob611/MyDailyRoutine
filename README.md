...
- Type scale ported from Apple's Dynamic Type instead of Material's defaults — Large Title 34/41, Title 1 28/34, Title 2 22/28, Title 3 20/25, Headline 17 semibold, Body 17/22, Callout 16/21, Subhead 15/20, Footnote 13/18, Caption 12/16 — with Regular body copy and Semibold reserved for emphasis, SF Pro's variable letter spacing (negative and growing with size), and the bundled Roboto Flex `opsz` axis set per size in the widget's TextView layouts, where Android exposes it (Compose's `TextStyle` has no font-variation parameter at all). Role names stay Material's, so all 300-odd call sites picked it up at once.
- Shared OLED palette — not hand-picked: `tools/derive_palette.py` computes every token from published constants (Apple's dark-mode semantic colours and system tints, Material 3's baseline dark scheme tones, WCAG 2.1) and fails CI if the shipped `DesignSystem.kt` drifts from the derivation; `tools/check_contrast.py` then verifies 157 text/surface/accent/glass pairs against per-role minimums (primary 11:1, secondary at WCAG AAA 7:1, muted 5.5:1, worst surface), with a halation guard that forbids both pure black and pure white, including the widget's XML mirror of the palette — hairline cards, bundled Roboto Flex, and tabular clock/countdown figures.
- Liquid-glass chrome (kyant0 `backdrop`) over full-bleed content — per-role material tiers (24-28 dp blur, refraction lens with chromatic dispersion, 160 % saturation lift, specular rim) — a specular hairline along the lit top edge of every floating panel (the detail that separates glass from tinted film), hairlines raised to Apple's dark-separator level (1.43:1 instead of 1.34:1), a top bar that folds while the reader scrolls, spring transitions that follow the direction of travel, a derived back stack (scale → Goals → sheets, Goals exiting on the predictive-back drag itself), an interactive fast-add control that shrinks under the finger and lights up at the touch point, Apple's spring vocabulary converted exactly (`k = (2π ÷ duration)²`, `ζ = 1 − bounce`) for glass chrome while Material springs keep moving content, and full respect for the system's *Remove animations* setting. See [the deep UI review](docs/audits/UI_DEEP_REVIEW.md).
...
- Live NOW spine, real 15-minute-step long-press rescheduling that ticks once per detent, and an opt-out haptic vocabulary mapped onto Apple's feedback generators — impact (light / medium / heavy), selection, notification (success / warning / error), gesture and switch — composed from hardware primitives on API 31+, predefined effects on API 29+, and OEM-tuned platform constants below that. Actions get their haptic from one wrapper that classifies them by meaning, so nothing fires twice.
